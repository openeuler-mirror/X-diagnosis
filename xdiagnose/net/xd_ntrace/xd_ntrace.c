...
char *  trace_result[IPPROTO_MAX][NET_MAX + 1] = {
    [IPPROTO_ICMPV6][NET_SKB_RCV] = "check virtual device(eg:vlan tag/bridge)",
    ...
    [IPPROTO_ICMPV6][NET_ICMP_RCV] = "check icmpv6_echo_ignore_all in sysctl",
...
static int parse_protostr(const char *port_str)
{
    if (!strcmp(port_str, "tcp"))
        return IPPROTO_TCP;
    else if (!strcmp(port_str, "udp"))
        return IPPROTO_UDP;
    else if (!strcmp(port_str, "icmp"))
        return IPPROTO_ICMP;
    else if(!strcmp(port_str, "icmp6"))
        return IPPROTO_ICMPV6;
    else {
        printf("Invalid protovol(s):%s\n", port_str);	
        return -1;
    }
}
...
static void event_handler(void *ctx, int cpu, void *data, unsigned int data_sz)
{
    ...
    if (event->icmp_type == ICMP_ACTIVE) {
        ...
        printf("active ping: [%s->%s] cpu[%d] queue[%d] icmp_seq=%u %s\n", 
              sip, dip, event->cpuid, event->queue,
              event->icmp_seq,trace_result[protocol][stage]);
    } else if (event->icmp_type == ICMP_PASSIVE) {
        inet_ntop(filter.family, filter.hostaddr, dip, sizeof(dip));
        printf("passive ping: [%s->%s] cpu[%d] queue[%d] icmp_seq=%u time=%lu us %s\n",
              sip, dip, event->cpuid, event->queue,
              event->icmp_seq, event->ts,  trace_result[protocol][stage]);
    }
    return;
}
...
int main(int argc, char **argv)
{
    ...
    skel = xd_ntrace_bpf__open_and_load();
    if (!skel) {
    ...
    /* Attach tracepoint */
    ret = xd_ntrace_bpf__attach(skel);
    if (ret) {
	...
	pb = perf_buffer__new(bpf_map__fd(skel->maps.net_trace_event), \
