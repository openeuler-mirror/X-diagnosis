...
static int read_v6_l3(struct sk_buff *skb, struct ntrace_flow_keys *tuple, int offset, int stage, int *nextoff, unsigned short *protocol)
{
    unsigned  char *head;
    u16 l3_offset = 0;
    struct ipv6hdr ip6h = {0};
    struct frag_hdr fragh = {0};
    int offset_next = 0;
    ...
    if (!filter_l3(ip6h.saddr.in6_u.u6_addr32, ip6h.daddr.in6_u.u6_addr32)) {
        return 0;
    }
    if (proto == IPPROTO_TCP || proto == IPPROTO_UDP) {
        __builtin_memcpy(tuple->key.tp.saddr, ip6h.saddr.in6_u.u6_addr32, sizeof(ip6h.saddr.in6_u.u6_addr32));
        __builtin_memcpy(tuple->key.tp.daddr, ip6h.daddr.in6_u.u6_addr32, sizeof(ip6h.daddr.in6_u.u6_addr32));
...
static void parse_icmpv6(void *ctx, struct sk_buff *skb, struct ntrace_flow_keys *tuple, int stage, int offset)
{
    ...
    if (offset) {
        bpf_probe_read((void *)&icmphdr, sizeof(icmphdr), (void*)(skb_head + offset));
    } else {
        bpf_probe_read(&l4_offset, sizeof(l4_offset), &skb->transport_header);
        bpf_probe_read(&icmphdr, sizeof(icmphdr), skb_head + l4_offset);
    }
    if (icmphdr.icmp6_type != ICMPV6_ECHO_REQUEST && icmphdr.icmp6_type != ICMPV6_ECHO_REPLY)
        return;
    tuple->key.icmp_key.icmp_id = bpf_htons(icmphdr.icmp6_dataun.u_echo.identifier);
    if (!offset && !read_v6_l3(skb, tuple, offset, stage, &nextoff, &proto)) {
        return;
    }
    value = bpf_map_lookup_elem(&sockresult_map, tuple);
    if (value) {
        if (icmphdr.icmp6_type == ICMPV6_ECHO_REPLY && stage == NET_DEV_START_XMIT) {
            value->ts = (bpf_ktime_get_ns() - value->ts) / 1000;
        }
        if (stage == NET_SKB_RCV && icmphdr.icmp6_type == ICMPV6_ECHO_REQUEST && value->icmp_type == ICMP_PASSIVE) {
            __builtin_memcpy(&event, value, sizeof(struct ntrace_put_user_info));
        ...
        value->stage = stage;
        value->icmp_seq = bpf_htons(icmphdr.icmp6_dataun.u_echo.sequence);
    } else {
create_map:
        if (stage != NET_SKB_RCV || icmphdr.icmp6_type != ICMPV6_ECHO_REQUEST)
            return;
        ...
        if (!value)
            return;
        value->stage = stage;
        bpf_probe_read_str(&value->indev_name, sizeof(value->indev_name), netdev->name);
...
static void v6_prase_l3(void *ctx, struct sk_buff *skb, struct ntrace_flow_keys *tuple, int stage, int offset)
{
    u16 l3_offset = 0;
    int offset_next = 0;
...
static void parse_l2(void *ctx, struct sk_buff *skb, struct ntrace_flow_keys *tuple, int stage)
{
    int offset = 0;
    u16 proto;
    char *head;
    ...
    if ((proto == bpf_htons(ETH_P_8021AD)) || (proto == bpf_htons(ETH_P_8021Q))) {
        bpf_probe_read(&head, sizeof(head), &skb->head);
        bpf_probe_read(&network_header, sizeof(network_header), &skb->network_header);
        bpf_probe_read(&proto, sizeof(proto), head + network_header + offsetof(struct vlan_hdr,
				h_vlan_encapsulated_proto));
        offset = sizeof(struct vlan_hdr);// 处理vlan
    }
    switch (bpf_htons(proto)) {
        case ETH_P_IP:
            // v4_parse_l3(ctx, skb, tuple, stage, offset);
	    break;
        case ETH_P_IPV6:
            v6_prase_l3(ctx, skb, tuple, stage, offset);
	    break;
        default:
            break;
    }
...
int ntrace_rawv6_sendmsg(struct pt_regs *ctx)
{
    ...
    sk = (struct sock *)isk;
    bpf_probe_read(&sin6, sizeof(icmph), &msg->msg_name);
    ...
    bpf_probe_read(&icmp_seq, sizeof(icmp_seq),&icmph->icmp6_dataun.u_echo.sequence);
    bpf_probe_read(&tuple.key.icmp_key.icmp_id, sizeof(tuple.key.icmp_key.icmp_id),&icmph->icmp6_dataun.u_echo.identifier);
    ...
    bpf_probe_read(value->saddr, sizeof(value->saddr), pinet6->saddr.in6_u.u6_addr32);
    bpf_probe_read(&sk_sndbuf, sizeof(sk_sndbuf), &sk->sk_sndbuf);
    bpf_probe_read(&sk_wmem_alloc, sizeof(sk_wmem_alloc), &sk->sk_wmem_alloc);
...
int ntrace_dev_hard_start_xmit(struct pt_regs *ctx)
{
    ...
    v6_prase_l3(ctx, skb, &tuple, NET_DEV_START_XMIT, 0);
    value = bpf_map_lookup_elem(&sockresult_map, &tuple);
...
int ntrace_sch_direct_xmit(struct pt_regs *ctx)
{
    ...
    v6_prase_l3(ctx, skb, &tuple, NET_DEV_SCH_DIR_XMIT, 0);
    value = bpf_map_lookup_elem(&sockresult_map, &tuple);
    if (value && txq) {
        ...
        bpf_probe_read(&value->queue, sizeof(skb->queue_mapping), &skb->queue_mapping);
	bpf_probe_read(&value->tc_drop, sizeof(q->qstats.drops), &q->qstats.drops);
    }
    return 0;
}
...
{
    struct ntrace_flow_keys tuple = {0};
    v6_prase_l3(args, args->skbaddr, &tuple, NET_DEV_QUEUE_XMIT, 0);
    return 0;
}
...
int ntrace_ip6_finish_output2(struct pt_regs *ctx)
{
    ...
    v6_prase_l3(ctx, skb, &tuple, NET_IP_FINISH_OUTPUT2, 0);
    return 0;
}
...
int ntrace_ip6_finish_output(struct pt_regs *ctx)
{
    ...
    struct ntrace_flow_keys tuple = {0};
    skb = (struct sk_buff *)PT_REGS_PARM3(ctx);
    v6_prase_l3(ctx, skb, &tuple, NET_IP_FINISH_OUTPUT, 0);
    return 0;
}
...
int ntrace_ip6_output(struct pt_regs *ctx)
{
    ...
    struct ntrace_flow_keys tuple = {0};
    skb = (struct sk_buff *)PT_REGS_PARM3(ctx);
    v6_prase_l3(ctx, skb, &tuple, NET_IP_OUTPUT, 0);
    return 0;
}
...
int ntrace_ip6_local_out(struct pt_regs *ctx)
{
    ...
    struct ntrace_flow_keys tuple = {0};
    skb = (struct sk_buff *)PT_REGS_PARM3(ctx);
    v6_prase_l3(ctx, skb, &tuple, NET_IP_LOCAL_OUTPUT, 0);
    return 0;
}
...
int ntrace_ip6_protocol_deliver(struct pt_regs *ctx)
{
    ...
    struct ntrace_flow_keys tuple = {0};
    skb = (struct sk_buff *)PT_REGS_PARM2(ctx);
    v6_prase_l3(ctx, skb, &tuple, NET_IP_INPUT_FINISH, 0);
    return 0;
}
...
int ntrace_ip6_input(struct pt_regs *ctx)
{
    ...
    struct ntrace_flow_keys tuple = {0};
    skb = (struct sk_buff *)PT_REGS_PARM1(ctx);
    v6_prase_l3(ctx, skb, &tuple, NET_IP_INPUT, 0);
    return 0;
}
...
int ntrace_ip6_rcv_finish(struct pt_regs *ctx)
{
    ...
    struct ntrace_flow_keys tuple = {0};
    skb = (struct sk_buff *)PT_REGS_PARM1(ctx);
    v6_prase_l3(ctx, skb, &tuple, NET_IP_ROUTE_INPUT, 0);
    return 0;
}
...
int ntrace_ip_rcv_core(struct pt_regs *ctx)
{
    ...
    struct ntrace_flow_keys tuple = {0};
    skb = (struct sk_buff *)PT_REGS_PARM1(ctx);
    v6_prase_l3(ctx, skb, &tuple, NET_IP_RCV_CORE, 0);
    return 0;
}
