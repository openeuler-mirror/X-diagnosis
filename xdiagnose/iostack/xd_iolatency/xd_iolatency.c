...
static void print_issue_stat(struct iolatency_ctx *ctx)
{
	...
	struct xd_iolatency_bpf *obj = ctx->obj;
...
static int xd_iolatency_section_attach(struct xd_iolatency_bpf *obj)
{
	int err = 0;
	int size, i;
...
int main(int argc, char **argv)
{
	...
	obj = xd_iolatency_bpf__open();
	if (!obj) {
		fprintf(stderr, "failed to open BPF object\n");
		return 1;
	}

	err = xd_iolatency_bpf__load(obj);
	if (err) {
