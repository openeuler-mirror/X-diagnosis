...
static void ext4fsstat_data_clean(struct xd_ext4fsstat_bpf *skel)
{
	int err;
...
static void print_ext4fstat(struct xd_ext4fsstat_bpf *skel)
{
	int rows = 0;
...
static void set_filter(struct xd_ext4fsstat_bpf *skel)
{
	int err;
	__u32 dev;
...
static int ext4fsstat_attach_read(struct xd_ext4fsstat_bpf *skel)
{
	int err = 0;
...
static int ext4fsstat_attach_write(struct xd_ext4fsstat_bpf *skel)
{
	int err = 0;
...
static int xd_ext4fstat_progs_attach(struct xd_ext4fsstat_bpf *skel)
{
	int err = 0;
...
static void poll_process_ext4fstat(struct xd_ext4fsstat_bpf *skel)
{
...
int main(int argc, char **argv)
{
	...
	skel = xd_ext4fsstat_bpf__open();
	if (!skel) {
		fprintf(stderr, "failed to open BPF object\n");
		return 1;
	}

	err = xd_ext4fsstat_bpf__load(skel);
	if (err) {
...
cleanup:
	xd_ext4fsstat_bpf__destroy(skel);
	return err < 0 ? -err : 0;
}	
