...
static struct xd_schedmonitor_bpf_c *skel;
...
#endif
	skel = xd_schedmonitor_bpf_c__open_and_load();
	if (!skel) {
		fprintf(stderr, "Failed to open and load BPF skeleton\n");
		return -1;
	}   

	ret = xd_schedmonitor_bpf_c__attach(skel);
	if (ret) {
...
cleanup:
	...
	if(args_mapfd > 0)
		close(args_mapfd);
	xd_schedmonitor_bpf_c__destroy(skel);
	return ret;
}
