...
static void set_filter_rule(struct xd_scsiiotrace_bpf_c *skel, struct env *env)
{
...
static int xd_scsiiotrace_bpf_c_progs_attach(struct xd_scsiiotrace_bpf_c *skel)
{
	int err = 0;

	if (!env.filter_event)
		return xd_scsiiotrace_bpf_c__attach(skel);
...
perf_buffer_create(struct xd_scsiiotrace_bpf_c *skel)
{
...
{
	struct perf_buffer *pb = NULL;
	struct xd_scsiiotrace_bpf_c *skel;
	int err;
	...
	skel = xd_scsiiotrace_bpf_c__open_and_load();
	if (!skel) {
	...
	err = xd_scsiiotrace_bpf_c_progs_attach(skel);
	if (err) {
...
cleanup:
	...
	xd_scsiiotrace_bpf_c__destroy(skel);
