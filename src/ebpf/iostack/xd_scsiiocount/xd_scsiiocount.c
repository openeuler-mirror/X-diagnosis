...
static void set_filter_disk(struct xd_scsiiocount_bpf_c *skel,
...
static void poll_process_sdev_io(struct xd_scsiiocount_bpf_c *skel,
				 struct disk_info *disk)
{
...
{
	struct xd_scsiiocount_bpf_c *skel;
	struct disk_info *disk;
	int err;
	...
	skel = xd_scsiiocount_bpf_c__open();
	if (!skel) {
	...
	err = xd_scsiiocount_bpf_c__load(skel);
	if (err) {
	...
	err = xd_scsiiocount_bpf_c__attach(skel);
	if (err) {
...
cleanup:
	xd_scsiiocount_bpf_c__destroy(skel);
