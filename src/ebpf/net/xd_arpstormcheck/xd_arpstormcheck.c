...
#include "bpf/xd_arpstormcheck.h"
#include "common_u.h"
...
int main(int argc, char **argv)
{
	int ch;
	int ret = 0;
	struct xd_arpstormcheck_bpf_c *skel;
	...
	skel = xd_arpstormcheck_bpf_c__open_and_load();
	if (!skel) {
	...
	/* Attach tracepoint */
	ret = xd_arpstormcheck_bpf_c__attach(skel);
	if (ret) {
...
cleanup:
	xd_arpstormcheck_bpf_c__destroy(skel);
	return ret;
}
