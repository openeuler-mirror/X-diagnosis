...
#include "bpf/xd_netvringcheck.h"
#include "common_u.h"
...
int main(int argc, char **argv)
{
	int ch;
	int ret = 0;
	struct xd_netvringcheck_bpf_c *skel;
	...
	skel = xd_netvringcheck_bpf_c__open_and_load();
	if (!skel) {
	...
	/* Attach tracepoint */
	ret = xd_netvringcheck_bpf_c__attach(skel);
	if (ret) {
...
cleanup:
	xd_netvringcheck_bpf_c__destroy(skel);
	return ret;
}
