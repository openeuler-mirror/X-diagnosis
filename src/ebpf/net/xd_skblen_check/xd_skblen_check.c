...
int main(int argc, char **argv)
{
    int ch;
    int ret = 0;
    struct xd_skblen_check_bpf_c *skel;
    ...
    skel = xd_skblen_check_bpf_c__open_and_load();
    if (!skel) {
    ...
    /* Attach tracpoint */
    ret = xd_skblen_check_bpf_c__attach(skel);
    if (ret) {
    ...
    close(map_fd);
cleanup:
    xd_skblen_check_bpf_c__destroy(skel);
    return ret;
}
